...
EVENT_STYLE = {
    ...
    # ── Ari Logic Trade Levels ──
    "ari_long": ("🟢", "New LONG entry signal (EMA 9/21 cross, HTF-aligned)."),
    "ari_short": ("🔴", "New SHORT entry signal (EMA 9/21 cross, HTF-aligned)."),
    "ari_target": ("🎯", "Final target reached — bank it."),
    "ari_stop": ("⛔", "Stop hit — plan invalidated, step back."),
    "ari_exit": ("🟠", "Exit warning — momentum/flow turned against your side."),
    "ari_reversal": ("⚠️", "Early reversal — opposite side gaining confluence."),
    "ari_flow_conflict": ("🟡", "Order flow is opposing the current bias."),
    "ari_full_align": ("✅", "Full confluence — STF + HTF + flow all aligned."),
    "ari_daily_limit": ("🛑", "Daily risk limit reached — stop trading for the day."),
}
...
def fmt(data: dict) -> str:
    ...
    side = data.get("side")
    openr = data.get("openR")
    grade = data.get("grade")

    parts = [f"{emoji} ORBIT · {sym}", line]
    facts = []
    if side is not None:
        facts.append(f"Side: {side}")
    ...
    if grade is not None:
        facts.append(f"Grade: {grade}")
    if openr is not None:
        facts.append(f"Open: {openr}R")
    if nxt is not None:
